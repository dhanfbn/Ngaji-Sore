...
import type { MurojaahLogRow, MurojaahPencapaianLevel } from '@/types/dashboard';

interface MurojaahLogTableProps {
  monthLabel: string;
  log: MurojaahLogRow[];
}

const PENCAPAIAN_DISPLAY: Record<MurojaahPencapaianLevel, { className: string }> = {
...
// ── Component ──────────────────────────────────────────────────
export function MurojaahLogTable({ monthLabel, log }: MurojaahLogTableProps) {
  return (
    <Card className="card-3d overflow-hidden bg-white">
      <CardContent className="p-6 sm:p-8">
        <div className="inline-flex items-center gap-2 bg-slate-700 rounded-full pl-1 pr-4 py-1 shadow-sm mb-6 self-start shrink-0">
          ...
          <span className="text-xs font-extrabold uppercase tracking-wide text-white">Log Murojaah Harian ({monthLabel})</span>
        </div>
            ...
            <table className="w-full text-sm sm:text-base border-collapse min-w-[640px]">
              <thead className="sticky top-0 bg-white">
                <tr className="border-b-2 border-border">
                  ...
                  <th className="text-left font-bold text-muted-foreground text-xs uppercase tracking-wide py-3 pr-4">Surah Diulang</th>
                  <th className="text-left font-bold text-muted-foreground text-xs uppercase tracking-wide py-3 pr-4">Status Kelancaran</th>
                ...
                {log.map(row => {
                  ...
                  return (
                    <tr key={row.no} className="border-b border-border last:border-0 hover:bg-slate-50/70">
                      ...
                      <td className="py-3.5 pr-4 font-semibold text-foreground whitespace-nowrap">{row.suratDiulang}</td>
                      <td className="py-3.5 pr-4 whitespace-nowrap">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold ${pencapaian.className}`}>
                          {row.statusKelancaran}
                        </span>
                      </td>
...
// ── Skeleton ───────────────────────────────────────────────────
export function MurojaahLogTableSkeleton() {
  return (
    <Card className="card-3d overflow-hidden bg-white">
